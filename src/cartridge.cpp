...
cartridge::cartridge(char* file_name){
	...
	if(fd.is_open()) {
		...
		//_unused_data = (byte_t*) malloc(UNUSED_SIZE * sizeof(byte_t));
		//fd.read((char*)_unused_data, UNUSED_SIZE);
		
		_prg_data = (byte_t*) malloc(PRG_BANK_SIZE * sizeof(byte_t));
		fd.read((char*)_prg_data, _header->get_n_prg_banks() * PRG_BANK_SIZE);
		
		_chr_data = (byte_t*) malloc(CHR_BANK_SIZE * sizeof(byte_t));
...
byte_t cartridge::read(ui16_t address){
	return _prg_data[_mapper->map_prg_address(address)];
}

void cartridge::write(ui16_t address, byte_t byte){
