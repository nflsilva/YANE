...
u2c02::u2c02(ppu_bus* b) : _ppustatus(0xE0), _address_latch(false), 
...
void u2c02::clock(){
	...
	if(_current_cycle > -1 && _current_cycle < 257 && _current_scanline > -1 && _current_scanline < 240){
		...
		switch(cycle_phase){
			case 0:
				...
				_pattern_low_bit = _bus->read(0x1000 + tile_id * 16 + _current_scanline % 8);
				_pattern_high_bit = _bus->read(0x1000 + tile_id * 16 + 8 + _current_scanline % 8);
				
				//_shifter_pattern_low = _pattern_low_bit;
				//_shifter_pattern_high = _pattern_high_bit;
	...
	// rendering limits
	_current_cycle++;
	if(_current_cycle == 257){
		_current_cycle = 0;
		_current_scanline++;
		_coarse_y = _current_scanline / 8;
		
		if(_current_scanline == 239){
			_current_scanline = 0;
			_coarse_y = 0;
